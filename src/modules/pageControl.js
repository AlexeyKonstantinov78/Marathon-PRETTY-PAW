const showSection = () => {
  ...
  const sections = document.querySelectorAll('.page__content');
  const links = document.querySelectorAll('.navigation__link, .page__nav-link');
  ...
  for (const link of links) {
    ...
    if (link.classList.contains('page__nav-link')) {
      classActive = `page__nav-link_active`;
    }
